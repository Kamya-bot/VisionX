...
async def get_drift_status(
    ...
    **Interview Talking Point:**
    ...
    try:
        ...
        return _numpy_safe(drift_report)
...
async def get_drift_summary(
    ...
    try:
        ...
        return {
            ...
            'drifted_features_count': int(summary.get('feature_drift_count', 0)),
            'prediction_psi': round(float(summary.get('prediction_psi', 0.0)), 4),
...
async def get_feature_drift_detail(
    ...
    try:
        ...
        return {
            ...
            'drift_detected': bool(detail['is_drift']),
            'severity': detail['severity'],
            'statistics': {
                'ks_statistic': round(float(detail['ks_statistic']), 4),
                'p_value': round(float(detail['p_value']), 6),
                'mean_shift': round(float(detail['mean_shift']), 4),
                'std_shift': round(float(detail['std_shift']), 4)
            },
            'baseline': {
                'mean': round(float(detail['baseline_mean']), 4),
                'std': round(float(detail['baseline_std']), 4)
            },
            'current': {
                'mean': round(float(detail['current_mean']), 4),
                'std': round(float(detail['current_std']), 4)
            },
...
async def get_drift_alerts(
    ...
    try:
        ...
        if drift_data.get('status') == 'insufficient_data':
            return {'alerts': [], 'message': drift_data.get('message')}
        ...
        if prediction_drift.get('is_drift'):
            alerts.append({
                ...
                'psi_score': round(float(prediction_drift['psi_score']), 4),
...
def _interpret_drift(detail: dict) -> str:
    ...
    mean_shift = float(detail['mean_shift'])
    severity = detail['severity']
